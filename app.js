...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(compareResultsBtn) {
        compareResultsBtn.addEventListener('click', openModal);
    }
    ...
    document.querySelectorAll('[data-sort-col]').forEach(th => {
        th.addEventListener('click', () => sortTable(th.dataset.sortCol));
        th.addEventListener('keydown', (event) => handleKeySort(event, th.dataset.sortCol));
    });

    const searchBtn = document.getElementById('searchGoogleBtn');
    const researchPromptInput = document.getElementById('researchPrompt');

    if (searchBtn && researchPromptInput) {
        searchBtn.addEventListener('click', performGoogleSearch);
        researchPromptInput.addEventListener('keypress', (e) => {
